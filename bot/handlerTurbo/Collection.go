...
func LoadCollection(redisClient *redis.Client, userId int64) (collection Collection, err error) {
    ...
    collection.data, err = redisClient.ZRangeWithScores(collection.redisKey, 0, -1).Result()
...
func (c Collection) MoveTo(collection Collection, liner Liner) (err error) {
    if !c.Has(liner) {
        return errors.New(fmt.Sprintf("В твоей коллекции нет вкладыша с номером '%s'", liner))
    }
